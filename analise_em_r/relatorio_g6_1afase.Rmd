...
install.packages("DESeq2") # isso faz parte do cBioPortal
...
# Configuração inicial para o documento
...
library(ggplot2)
library(dplyr)
library(DESeq2) #isso faz parte do cBioPortal
library(edgeR)
library(limma)
library(pheatmap)
...
# 5.3. DESeq2 - Expressão Diferencial
...
```{r}
counts <- assay(geneExp_df)
colData <- colData(geneExp_df)
dds <- DESeqDataSetFromMatrix(countData = counts, colData = colData, design = ~ vital_status)
dds <- dds[rowSums(counts(dds)) >= 10, ]
dds <- DESeq(dds)
res_deseq <- results(dds)
...
# 5.4. edgeR - Expressão Diferencial
...
# 5.5 limma-voom - Expressão Diferencial
...
# 5.7. Heatmap dos Top 30 Genes (DESeq2)
...
# 5.8. Enriquecimento Funcional (GO + KEGG)
